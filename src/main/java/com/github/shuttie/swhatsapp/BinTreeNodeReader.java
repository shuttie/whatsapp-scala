...
import com.github.shuttie.swhatsapp.exceptions.DecodeException;
import com.github.shuttie.swhatsapp.exceptions.IncompleteMessageException;
import com.github.shuttie.swhatsapp.exceptions.InvalidMessageException;
import com.github.shuttie.swhatsapp.exceptions.InvalidTokenException;
...
public class BinTreeNodeReader {
     ...
     * @throws com.github.shuttie.swhatsapp.exceptions.DecodeException
    ...
    public ProtocolNode nextTree(byte[] readData) throws IncompleteMessageException, InvalidMessageException, InvalidTokenException, IOException, InvalidKeyException, NoSuchAlgorithmException, DecodeException {
        if(readData != null) {
            input = readData;
            //System.out.println("Input ="+BinHex.bin2hex(readData));
        }
...
//	    			log.debug("Input decoded ="+BinHex.bin2hex(decoded));
    ...
    private ProtocolNode nextTreeInternal() throws InvalidTokenException, IOException {
        ...
        if (token == 1) {
            ...
            return ProtocolNode.apply("start", attributes);
        }
        if (token == 2) {
            return null;
        }
        ...
        Map<String, String> attributes = readAttributes(size);
        if ((size % 2) == 1) {
            return ProtocolNode.apply(tag, attributes);
        }
        token = readInt8();
        if (isListTag(token)) {
            return ProtocolNode.apply(tag, attributes, readList(token));
        }

        return ProtocolNode.apply(tag, attributes, readString(token));
    }
...
//	      log.debug(string);
    ...
    private byte[] getToken(int token) throws InvalidTokenException {
        ...
        if(token >= 236) {
            subdict = true;
            token = readInt8();
            t = TokenMap.getByIndex(token, subdict);
        } else {
            t = TokenMap.getByIndex(token, subdict);
        }
        if (!t.isDefined()) {
